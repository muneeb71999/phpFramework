...
<div class="container-sm mt-5">
  <div class="row">
    ...
    <form method="post" action="<?= URLROOT ?>/numbers/update/<?= $data['id'] ?>">
      <div class="grid">
        ...
        <div class="form-group">
          ...
          <input type="number" value="<?= isset($data['customer_monthly']) ? $data['customer_monthly'] : '0' ?>" name="customer_monthly" class="form-control" id="customer_monthly">
        ...
        <div>
          ...
          <select class="form-select" name="person_id" aria-label="Default select example">
            <?php foreach ($data['persons'] as $person) : ?>
              <option <?= $data['person_id'] == $person['person_id'] ? 'selected' : ''; ?> value="<?= $person['person_id'] ?>"><?= $person['name'] ?></option>
            <?php endforeach; ?>
          </select>
        </div>
